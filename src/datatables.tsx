...
const DataTables: React.FC = () => {
  ...
  const [groupedData, setGroupedData] = useState<{ [key: string]: { typename: string; data: any[] } }>({});
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const currentUser = allUsers.find((user: any) => user.user_id === userId);
      const filteredUsers = allUsers.filter((user: any) => user.user_id !== userId);
...
const snapToTens = (v: number, mode: "floor" | "nearest" | "ceil" = "floor") => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setTotal(response.data.reduce((sum: number, item: any) => sum + item.total_amount, 0));
...
const tableContent = isMobile ? (
  <div className="mobile-card-stack">
    {adjustedRows.map((record: any, idx: number) => {
      const key = record.__isBucket ? `bucket-${record.bucketIndex}-${idx}` : `${record.inumber}-${idx}`;
      const value = record.__isBucket ? record.total_amount : record.adjusted_amount ?? record.total_amount;
      return (
        <div className={`mobile-card ${record.__isBucket ? "mobile-card--bucket" : ""}`} key={key}>
          <div className="mobile-card__row">
            <span className="mobile-card__label">{record.__isBucket ? "Bucket" : "Number"}</span>
            <span className="mobile-card__value">{record.__isBucket ? record.bucketLabel : record.inumber}</span>
          </div>
          <div className="mobile-card__row">
            <span className="mobile-card__label">{record.__isBucket ? "Total" : "Amount"}</span>
            <span className="mobile-card__value">{formatNumber(value)}</span>
          </div>
        </div>
      );
    })}
  ...
  <Table
    ...
    pagination={false}
    scroll={isMobile ? undefined : { y: 240 }}
